...
public class CommemorationSignBlockSchematic implements IBlockSchematic
{
    ...
    public void setDirection(BlockFace direction)
    {
        ...
        for (BlockFace supportedDirection : getSupportedDirections())
        {
            if (direction == supportedDirection)
                isDirectionSupported = true; break;
    ...
    @Override
    public BlockFace[] getSupportedDirections()
    {
    ...
    private void generateBlockLocations(BlockFace direction)
    {
        ...
        {
            locBlockMap.put(new Location(null, 1, 0, 0), Material.OAK_FENCE);
            locBlockMap.put(new Location(null, 1, 1, 0), Material.OAK_FENCE);
            locBlockMap.put(new Location(null, 0, 1, 0), Material.AIR);
        }
        else if (direction == BlockFace.NORTH)
        {
            locBlockMap.put(new Location(null, 0, 0, 1), Material.OAK_FENCE);
            locBlockMap.put(new Location(null, 0, 1, 1), Material.OAK_FENCE);
            locBlockMap.put(new Location(null, 0, 1, 0), Material.AIR);
        }
